package uk.gov.hmcts.reform.sscs.partiesConsumer;
...
public class HearingPartiesPutConsumerTest extends ContractTestDataProvider {
    ...
    public RequestResponsePact putHearingRequestForValidRequest(PactDslWithProvider builder) {
        return builder.given(CONSUMER_NAME + " successfully putting a hearing request ").uponReceiving(
                "Request to put hearing request")
            .path(HEARING_PATH).method(HttpMethod.PUT.toString()).query(ID + "=" + VALID_CASE_ID)
    ...
    @Pact(consumer = CONSUMER_NAME)
    public RequestResponsePact badRequestErrorFromPutHearing(PactDslWithProvider builder) {
        return builder.given(CONSUMER_NAME
                                 + " throws bad request error while trying to put hearing")
            .uponReceiving("Request to PUT hearing for bad hearing request")
    ...
    public RequestResponsePact unauthorisedRequestErrorFromPutHearing(PactDslWithProvider builder) {
        return builder.given(CONSUMER_NAME
                                 + " throws unauthorised error while trying to put hearing")
            .uponReceiving("Request to PUT hearing for unauthorised hearing request")
            .path(HEARING_PATH).method(HttpMethod.PUT.toString())
            .query(ID + "=" + VALID_CASE_ID)
    ...
    public RequestResponsePact forbiddenRequestErrorFromPutHearing(PactDslWithProvider builder) {
        return builder.given(CONSUMER_NAME
                                 + " throws forbidden error while trying to put hearing")
            .uponReceiving("Request to PUT hearing for forbidden hearing request")
            .path(HEARING_PATH).method(HttpMethod.PUT.toString())
    ...
    public RequestResponsePact notFoundRequestErrorFromPutHearing(PactDslWithProvider builder) {
        return builder.given(CONSUMER_NAME
                                 + " throws not found request error while trying to put hearing")
            .uponReceiving("Request to PUT hearing for not found hearing request")
            .path(HEARING_PATH).method(HttpMethod.PUT.toString())
    ...
    @Test
    @PactTestFor(pactMethod = "badRequestErrorFromPutHearing")
