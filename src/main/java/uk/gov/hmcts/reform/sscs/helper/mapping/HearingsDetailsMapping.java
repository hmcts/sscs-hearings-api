    ...
    public static HearingWindow buildHearingWindow(SscsCaseData caseData, boolean autoListed) {
        ...
        if (autoListed && nonNull(caseData.getEvents())) {
            Event dwpResponded = caseData.getEvents().stream()
                    .filter(c -> nonNull(c.getValue()) && EventType.DWP_RESPOND.equals(c.getValue().getEventType()))
                    .findFirst().orElse(null);
            if (nonNull(dwpResponded) && isNotBlank(dwpResponded.getValue().getDate())) {
                if (isYes(caseData.getUrgentCase())) {
                    dateRangeStart = dwpResponded.getValue().getDateTime().plusDays(14).toLocalDate();
                } else {
                    dateRangeStart = dwpResponded.getValue().getDateTime().plusMonths(28).toLocalDate();
                }
            }
        }
